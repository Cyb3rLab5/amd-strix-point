...
class FramePackTransformer(ModelMixin, ConfigMixin, PeftAdapterMixin, FromOriginalModelMixin):
    ...
    def initialize_teacache(self, enable_teacache=True, num_steps=25, rel_l1_thresh=0.15):
        ...
        # Use a list of coefficients instead of np.poly1d to avoid significant NumPy overhead
        # when evaluating a scalar value during the forward loop
    ...
    def _eval_poly(self, x):
        """Evaluate polynomial using Horner's method for better scalar performance than np.poly1d."""
        res = 0.0
        for c in self.teacache_rescale_coeffs:
            res = res * x + c
        return res
    ...
    def forward(
        ...
        if self.enable_teacache:
            ...
            if self.cnt == 0 or self.cnt == self.num_steps-1:
                ...
                self.accumulated_rel_l1_distance += self._eval_poly(curr_rel_l1)
