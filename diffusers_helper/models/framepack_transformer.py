    ...
    def forward(
        ...
        if self.enable_teacache:
            ...
            if self.cnt == 0 or self.cnt == self.num_steps-1:
                ...
                # Performance Optimization: evaluate poly directly instead of using np.poly1d in hot loop
                c = self.teacache_rescale_coeffs
                rescale_val = (((c[0] * curr_rel_l1 + c[1]) * curr_rel_l1 + c[2]) * curr_rel_l1 + c[3]) * curr_rel_l1 + c[4]
                self.accumulated_rel_l1_distance += rescale_val
