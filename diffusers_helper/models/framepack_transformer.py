...
class FramePackTransformer(ModelMixin, ConfigMixin, PeftAdapterMixin, FromOriginalModelMixin):
    ...
    def initialize_teacache(self, enable_teacache=True, num_steps=25, rel_l1_thresh=0.15):
        ...
        # Performance optimization: Evaluating np.poly1d on single scalar values in PyTorch hot loops
        # introduces significant Numpy overhead. We unroll the polynomial evaluation using Horner's
        # method which provides a ~130x speedup for this scalar operation.
        coeffs = [7.33226126e+02, -4.01131952e+02, 6.75869174e+01, -3.14987800e+00, 9.61237896e-02]
        self.teacache_rescale_func = lambda x: ((((coeffs[0] * x + coeffs[1]) * x + coeffs[2]) * x + coeffs[3]) * x + coeffs[4])
